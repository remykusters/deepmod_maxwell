...
def train_mse(model, data, target, optimizer, sparsity_scheduler, log_dir=None, max_iterations=10000, **convergence_kwargs):
    start_time = time.time()
    number_of_terms = [coeff_vec.shape[0] for coeff_vec in model(data)[3]]
    board = Tensorboard(number_of_terms, log_dir)  # initializing custom tb board
    ...
    for iteration in torch.arange(0, max_iterations + 1):
        ...
        loss = torch.sum(2 * torch.log(2 * pi * MSE))  # 1e-5 for numerical stability
        ...
        if iteration % 25 == 0:
            ...
            board.write(iteration, loss, MSE, MSE, l1_norm, constraint_coeff_vectors, unscaled_constraint_coeff_vectors)
